...
export default class TestInfiniteScrollUpdatePage {
  pageTitle: ElementFinder = element(by.id('jhipster180901App.testInfiniteScroll.home.createOrEditLabel'));
  saveButton: ElementFinder = element(by.id('save-entity'));
  cancelButton: ElementFinder = element(by.id('cancel-save'));
  userOneToManySelect: ElementFinder = element(by.css('select#test-infinite-scroll-userOneToMany'));
  userManyToManySelect: ElementFinder = element(by.css('select#test-infinite-scroll-userManyToMany'));
  userOneToOneSelect: ElementFinder = element(by.css('select#test-infinite-scroll-userOneToOne'));
