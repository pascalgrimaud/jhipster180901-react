...
export default class TestServiceImplUpdatePage {
  pageTitle: ElementFinder = element(by.id('jhipster180901App.testServiceImpl.home.createOrEditLabel'));
  saveButton: ElementFinder = element(by.id('save-entity'));
  cancelButton: ElementFinder = element(by.id('cancel-save'));
  userOneToManySelect: ElementFinder = element(by.css('select#test-service-impl-userOneToMany'));
  userManyToManySelect: ElementFinder = element(by.css('select#test-service-impl-userManyToMany'));
  userOneToOneSelect: ElementFinder = element(by.css('select#test-service-impl-userOneToOne'));
