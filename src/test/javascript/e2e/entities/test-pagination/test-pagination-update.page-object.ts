...
export default class TestPaginationUpdatePage {
  pageTitle: ElementFinder = element(by.id('jhipster180901App.testPagination.home.createOrEditLabel'));
  saveButton: ElementFinder = element(by.id('save-entity'));
  cancelButton: ElementFinder = element(by.id('cancel-save'));
  userOneToManySelect: ElementFinder = element(by.css('select#test-pagination-userOneToMany'));
  userManyToManySelect: ElementFinder = element(by.css('select#test-pagination-userManyToMany'));
  userOneToOneSelect: ElementFinder = element(by.css('select#test-pagination-userOneToOne'));
