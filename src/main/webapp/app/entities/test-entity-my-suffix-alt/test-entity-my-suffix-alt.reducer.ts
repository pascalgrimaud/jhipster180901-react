...
export type TestEntityMySuffixAltState = Readonly<typeof initialState>;

// Reducer

export default (state: TestEntityMySuffixAltState = initialState, action): TestEntityMySuffixAltState => {
  switch (action.type) {
...
export const getEntities: ICrudGetAllAction<ITestEntityMySuffixAlt> = (page, size, sort) => ({
  type: ACTION_TYPES.FETCH_TESTENTITY_LIST,
  payload: axios.get<ITestEntityMySuffixAlt>(`${apiUrl}?cacheBuster=${new Date().getTime()}`)
});

export const getEntity: ICrudGetAction<ITestEntityMySuffixAlt> = id => {
  const requestUrl = `${apiUrl}/${id}`;
  return {
    type: ACTION_TYPES.FETCH_TESTENTITY,
    payload: axios.get<ITestEntityMySuffixAlt>(requestUrl)
  };
};

export const createEntity: ICrudPutAction<ITestEntityMySuffixAlt> = entity => async dispatch => {
...
export const updateEntity: ICrudPutAction<ITestEntityMySuffixAlt> = entity => async dispatch => {
  const result = await dispatch({
...
export const deleteEntity: ICrudDeleteAction<ITestEntityMySuffixAlt> = id => async dispatch => {
  const requestUrl = `${apiUrl}/${id}`;
  const result = await dispatch({
